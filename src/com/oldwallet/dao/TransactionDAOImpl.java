...
public class TransactionDAOImpl implements TransactionDAO {
	...
	private static final String UPDATE_COUPON = "UPDATE COUPONS SET REDEEMED_BY=?,REDEEMED_DATE=NOW(),COMPLETED_REDEMPTIONS=?, REDEEM_STATUS=? WHERE COUPON_CODE=?";
	...
	public boolean updateTransaction(Transaction transaction) {
		...
		if("COMPLETE".equalsIgnoreCase(transaction.getStatus())) {
			...
			int result1 = jdbcTemplate.update(UPDATE_COUPON,transaction.getUserEmail(),1,redeemStatus, encCode);
			if (result1 > 0) {
